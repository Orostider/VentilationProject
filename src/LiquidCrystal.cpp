...
void delayMicroseconds(unsigned int us)
{
	...
	long double period = ((long double)1 / Chip_RIT_GetBaseClock(LPC_RITIMER)) * 1000000; // yhden tikin aika sekuntteina
	uint64_t tickCount = us / period;
	// disable RIT – compare value may only be changed when RIT is disabled
	Chip_RIT_Disable(LPC_RITIMER);
	// set compare value to RIT
	Chip_RIT_SetCompareValue(LPC_RITIMER, tickCount);
